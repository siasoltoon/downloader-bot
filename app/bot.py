...
async def receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return await msg.edit_text(f"❌ Could not extract this URL.\n`{type(exc).__name__}`", parse_mode="Markdown")
    context.user_data["pending_url"] = url
    context.user_data["formats"] = {f.format_id: f for f in info["formats"]}
    buttons = [[InlineKeyboardButton(f.label, callback_data=f"fmt:{f.format_id}")] for f in info["formats"]]
    if not buttons:
...
async def choose(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    option = formats.get(fid)
    if not url or option is None:
    ...
    await DB.update_job(job_id, status="downloading", format_id=option.expression)
    await q.edit_message_text("⏳ Download started…")
    async with JOB_LIMIT:
        try:
            path = await asyncio.to_thread(DL.download, url, option.expression, job_id)
            ...
            await q.message.reply_text(f"❌ Download failed: {type(exc).__name__}")
